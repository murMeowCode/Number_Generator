...
async def get_statistics(
    ...
    # Добавляем логирование перед вызовом (для отслеживания запроса)
    logger.info(f"📥 ЗАПРОС ДАННЫХ для sequence_id: {sequence_id}")
    ...
    try:
        ...
        logger.info(f"✅ ДАННЫЕ ПОЛУЧЕНЫ ИЗ БД. Statistics ID: {result.statistics_id}")
        ...
        logger.info(f"🎯 ОТВЕТ СФОРМИРОВАН для {sequence_id}")
        return response_data
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"❌ ОШИБКА при получении statistics для {sequence_id}: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
