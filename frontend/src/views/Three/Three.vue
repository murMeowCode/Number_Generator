...
const setTestResults = (serverResults) => {
  // Если передано null, очищаем все тесты
  if (serverResults === null) {
    console.log('🧹 Очищаем результаты тестов')
    
    tests.value.forEach(test => {
      // Сбрасываем все тесты в исходное состояние
      test.status = 'pending'
      test.progress = 0
      test.result = ''
      test.skipReason = ''
      test.duration = null
      test.serverData = null
    })
    
    // Сбрасываем флаги анализа
    analysisComplete.value = false
    isAnalyzing.value = false
    hasSkippedTests.value = false
    
    console.log('✅ Все тесты очищены')
    return
  }

  // Если переданы данные сервера, обрабатываем их как раньше
